...
class ContactView(View):
    ...
    async def ticket(self, interaction: discord.Interaction, button: Button):
        ...
        if existing_ticket:
            await interaction.response.send_message(f"⚠️ Tu as déjà un ticket : {existing_ticket.mention}", ephemeral=True)
            return
        ...
        embed = discord.Embed(title=f"🎟️ Ticket Ouvert - {self.agent_name}", description=f"Salut {user.mention} ! 👋\nTu contactes **{self.agent_name}**, {self.agent_data['role']}.\nExplique ta demande ici.", color=discord.Color.purple())
